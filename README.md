...
`core` (math/RNG/stats + loader + config loader + the registry spine `.registry`) · `config` (`.cfg` value files) · `data` (parser + splayed HDB) · `state` (point-in-time market state `.state` — the single as-of door) · `curve` (curve engine `.curve` — derived features + shocks + snapshots) · `roll` (roll discipline `.roll` — as-of-only active contract + roll events + analytics-only continuous view) · `models` (BS/FDM core + commodity + stochastic-vol/jump + exotics) · `calibration` (IV/surface/curve/Kalman) · `analytics` (risk/VaR/scenarios/limits/portfolio/reporting) · `signals` (seasonality) · `regime` (market-state engine `.regime` + analogue library/risk memory `.regime.analogue`) · `execution` (daily fill-and-cost) · `backtest` (strategy engine + commodity suite + walk-forward + the event-driven replay engine `.backtest.replay`) · `evidence` (the evidence audit `.evidence` — deterministic pass/fail verifier on a replay run + fail-safe gatedRun) · `season` (curve/spread seasonality `.season` — causal same-month z, R16's signal) · `carry` (carry/storage economics `.carry` — implied carry, convenience yield, carry signal, inventory proxy) · `attribution` (PnL explain + bucketed curve risk `.attribution` — level/slope/curvature/carry/residual; a HIGH layer above backtest/) · `portfolio` (strategy allocator `.alloc`) · `gov` (research governance `.gov` — registry/ledger/deflated-Sharpe/gate cascade) · `factor` (curve factor decomposition `.factor` — PCA, the first capability on the spine) · `templates` (problem templates `.template` — research-shape plug-in) · `cards` (model cards `.cards` — knowledge plug-in: contract + edge + gov-derived validation + audit) · `workflow` (the bounded research loop `.workflow` — incl. `runReplay`, the R16 end-to-end replay-mode path) · `agents` (six bounded research-agent role prompts + orchestration contract) · `apps` (examples + demos). Reserved: `services`, `scripts` (partial). Load everything with `\l core/init.q`.
...
├── roll/          # roll discipline (.roll) — as-of-only active contract + roll events + analytics-only continuous view
...
├── backtest/      # strategy engine + commodity strategy suite + walk-forward + event-driven replay (.backtest.replay)
├── evidence/      # the evidence audit (.evidence) — deterministic pass/fail verifier on a replay run + fail-safe gatedRun
├── season/        # curve/spread seasonality (.season) — causal same-month z (R16's signal); distinct from signals/
├── carry/         # carry/storage economics (.carry) — implied carry, convenience yield, carry signal, inventory proxy
...
├── agents/        # six bounded research-agent role prompts + ORCHESTRATION.md (docs; not loaded by init)
