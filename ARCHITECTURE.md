...
A `.cfg` namespace is populated at startup: load `config/base.q`, then `config/{env}.q` to override, where the environment is selected by the `QPRICER_ENV` variable (`dev` / `test` / `prod`, default `dev`).
...
2. **Config layer.** Add `.cfg`; replace hardcoded constants module by module.
