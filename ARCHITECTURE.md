...
**Status: COMPLETE (v0.60 commodity, v0.61 equity hedge, v0.63 equity option legs).** `execution/execution.q` (`.exec.*`) is a generic daily fill-and-cost layer: `.exec.fill[order;ctx;cfg]` (pure) returns filled quantity (full unless a participation cap binds), an adverse fill price, and cost components (proportional = `rate*|filled|`; slippage = `|filled|*refPrice*bps/1e4`; optional fixed + size/ADV impact). The default config (`.cfg.exec`) is frictionless and reproduces the legacy flat-cost path byte-identically; realism is opt-in via a per-run `exec` sub-config. EVERY trade routes through it: the commodity backtest (order = vol-targeted Δposition, refPrice = front price), the equity hedge (shared helper `.strategy.__hedgeStep`/`__hedgeInit`, order = dollar notional, refPrice=1), and every equity OPTION-LEG trade (shared `.strategy.__legCost`, order = dollar notional, refPrice=1, so slippage is the premium-scaled option bid-ask — the dominant cost). The `deltaPV==stepPnl` identity is preserved throughout. Matched to daily settle data — a fill-and-cost model, not a tick/LOB simulator.
...
4. **Execution layer.** Build `execution/`; route the backtest through it instead of the flat cost rate. *(DONE — v0.60: commodity BT; v0.61: equity hedge leg; v0.63 (4c): equity option legs via `.strategy.__legCost` (premium-scaled slippage). Every trade on both desks now routes through `.exec.fill`, frictionless default byte-identical, deltaPV==stepPnl preserved.)*
...
6. **IPC services** *(optional, last)* — gateway + HDB service + workers, only if always-on / multi-core scale is actually needed. *(DEFERRED until production onboarding or hardware beyond the single dev laptop: IPC solves a multi-user / multi-core / always-on need that single-user batch research does not have. Worthwhile at production scale; parked until then.)*
7. **CI + scheduled pipeline.** *(DEFERRED until production / adequate hardware: cloud CI needs a licensed q on the runner plus real compute for the full suite + nightly ingest→calibrate→backtest→report. Worthwhile at production; parked until then. Locally the green suite is run by hand as the merge gate.)*

The high-value 80% for the research goal is steps 1-4 plus CI; steps 1-5 are now complete (4 fully, incl. option-leg execution costs). Steps 6-7 are deferred per the notes above.
