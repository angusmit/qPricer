\l lib/init.q
/ Independent revaluation accounting test.
...
trade:`tradeId`underlying`productType`exerciseStyle`optionType`strike`expiry`notional!(
    `MD_AC;`X;`equityOption;`european;`call;100f;0.25;1f);
...
stratCfg:@[stratCfg;(`modelBVolBump;`disagreementThreshold;`stepYears;`txnCostRate;`financingRate);:;(0.10;0.05;1f%252f;0.001;0.02)];

pathRows:0!pathTbl;
firstStep:first pathRows;
remainingRows:1_pathRows;
initialState:.strategy.modelDisagreement.init[trade;firstStep;bsModel;fdmCfg;stratCfg];
stepFnLocal:.strategy.modelDisagreement.step[;;trade;bsModel;fdmCfg;stratCfg];
...
-1 "PASS test_strategy_model_disagreement_accounting: independentResidual=",string[independentResidual],", flowResidual=",string[flowMaxAbsRes];
