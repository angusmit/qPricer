...
trade:`tradeId`underlying`productType`exerciseStyle`optionType`strike`expiry`notional!(
    `PR_AC;`X;`equityOption;`european;`call;100f;0.25;1f);
...
stratCfg:@[.strategy.defaultConfig `putRatioBackspread;(`stepYears;`financingRate;`txnCostRate);:;(1f%252f;0.02;0.001)];
pathRows:0!pathTbl;
firstStep:first pathRows;
remainingRows:1_pathRows;
initialState:.strategy.putRatioBackspread.init[trade;firstStep;bsModel;fdmCfg;stratCfg];
stepFnLocal:.strategy.putRatioBackspread.step[;;trade;bsModel;fdmCfg;stratCfg];
...
-1 "PASS test_strategy_put_ratio_backspread_accounting: indepResidual=",string[independentResidual];
