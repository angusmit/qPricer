...
/ Multi-day replay across 3 dates (2 consecutive pairs)
replay:.parser.barchart.multiDayReplay[optionTable;2024.01.02;2024.01.04;1f];
.testutil.assertTrue[(count replay)>0;"replay has rows"];

/ Check marketPnl = qty * 100 * (exit - entry)
firstRow:replay 0;
expectedPnl:1f*100f*firstRow[`exitMarketPrice]-firstRow`entryMarketPrice;
.testutil.assertNear[firstRow`marketPnl;expectedPnl;0.01;"marketPnl formula correct"];

/ Daily PnL
daily:.parser.barchart.dailyPnl replay;
.testutil.assertTrue[(count daily)>0;"daily PnL has rows"];
.testutil.assertTrue[`cumulativeMarketPnl in key daily 0;"has cumulative column"];

/ Verify cumulative is running sum
cumVals:daily`cumulativeMarketPnl;
dailyVals:daily`dailyMarketPnl;
expectedCum:sums dailyVals;
.testutil.assertNear[last cumVals;last expectedCum;0.01;"cumulative = running sum"];

/ Backtest summary
bSummary:.parser.barchart.backtestSummary replay;
.testutil.assertTrue[bSummary[`contractCount]=2;"2 contracts in replay"];
.testutil.assertTrue[not null bSummary`totalMarketPnl;"totalPnl populated"];

-1 "PASS test_barchart_multiday_replay: replayRows=",string[count replay],", dailyRows=",string count daily;
